...
class AppController extends Controller
{
    ...
    public function createContactAction(Request $request)
    {
        ...
        if (false === empty($data['phoneNumber'])) {
            $contact->setPhoneNumber($data['phoneNumber']);
        }

        if (false === empty($data['subject'])) {
            $contact->setSubject($data['subject']);
        }

        if (false === empty($data['message'])) {
            $contact->setPhoneNumber($data['message']);
        }

        if (false === empty($data['service'])) {
        ...
        $message = $this->get('translator')->trans('app.expertise.contact.create.success', [], 'app');
        ...
        return $this->redirect($request->query->get('redirect_url') ?? $this->generateUrl('app_homepage'));
    }
}
